        ...
        <div className='sticky top-0 z-[1] bg-[#001f54]'>
            <div className='max-w-screen-xl mx-auto flex justify-between items-center py-4 px-3 xl:px-0'>
                <div className='flex justify-center items-center cursor-pointer' onClick={() => navigate("/")}>
                    <div className='flex justify-center items-center size-14 rounded-full bg-white'>
                        <img src={logo} alt="header-logo" className='size-12 p-1' />
                    </div>
                    <h4 className='text-white text-sm font-axiSemiBold tracking-wider ml-2'>AsianHope IT <br /> Company</h4>
                </div>
                        ...
                        <ul className='items-center gap-8 lg:gap-12 block md:flex md:py-0 py-1'>
                            {navItems?.map((item, i) => (
                                <li key={i} onClick={() => {
                                    setOpen(true),
                                        navigate(item?.slug)
                                }} className='font-axiMedium text-base lg:text-lg text-[#001f54] hover:text-gray-400 cursor-pointer py-2 md:py-0'>{item?.label}</li>
                            ))}
                        </ul>
                    </div>
